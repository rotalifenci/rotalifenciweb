...
// =========================================================================
// 🎯 KESİN VE HATASIZ KATEGORİ & ÜNİTE EŞLEŞTİRME YARDIMCILARI
// =========================================================================

function matchesSubTabCategory(item, subTab) {
    if (!item) return false;
    const itemCat = String(item.category || "").trim().toLowerCase();
    const target = String(subTab || "").trim().toLowerCase();
    ...
    if (target === "etkinlikler") {
        return itemCat === "etkinlikler" || itemCat === "etkinlik" || itemCat === "foy";
    }
    if (target === "soru-bankasi") {
        return itemCat === "soru-bankasi" || itemCat === "soru" || itemCat === "test";
    }
    if (target === "denemeler") {
        return itemCat === "denemeler" || itemCat === "deneme";
    }
    if (target === "egitsel-oyunlar" || target === "oyunlar") {
        return itemCat === "egitsel-oyunlar" || itemCat === "oyunlar" || itemCat === "oyun" || (!itemCat && format.includes("oyun"));
    }
    if (target === "bilim-insanlari" || target === "bilimin-rotasi") {
        return itemCat === "bilim-insanlari" || itemCat === "bilimin-rotasi";
    }
...
function getMaterialTargetSection(item) {
    ...
    // 3. Laboratuvar
    if (cat === "laboratuvar" || title.includes("laboratuvar") || title.includes("lab ") || unit.includes("laboratuvar") || title.includes("deney") || title.includes("güvenlik sembol") || title.includes("güvenlik kural") || title.includes("malzeme")) {
        return "lab";
    }

    // 4. Genel konu dağılımı / işlenecek üniteler / ilk ders / sınıf konuları -> 1. Ünite
    if (title.includes("konuları") || title.includes("konulari") || title.includes("işlenecek") || title.includes("islenecek") || title.includes("ilk ders") || title.includes("tanıtım") || title.includes("tanitim")) {
        return "1";
    }

    // 5. Belirli bir ünite: 2'den 7'ye kadar kontrol et
    for (let u = 2; u <= 7; u++) {
        const uRegex = new RegExp(`(?:^|\\b|\\s)${u}\\s*[\\.:\\-_]?\\s*ünite`, 'i');
        const altRegex = new RegExp(`ünite\\s*${u}\\b`, 'i');
        if (uRegex.test(unit) || altRegex.test(unit) || uRegex.test(title) || altRegex.test(title) || unit.startsWith(`${u}.`)) {
            return String(u);
        }
    }
...
function renderCustomMaterialsSection(gradeNumber = "all", subTab = "all") {
    ...
    const items = customList.filter(item => {
        ...
        const categoryMatch = matchesSubTabCategory(item, subTab);
...
function switchGradeSubTab(gradeId, tabName, event) {
    if (event) {
        // Kullanıcı Ctrl, Cmd veya fare orta tuşu ile basmışsa yeni sekmede açılmasına doğal izin ver
...
// =========================================================================
// 🌟 MODERN DİKEY & KATEGORİZE AKORDEON BİLEŞENLERİ (MEB 5, 6, 7, 8. SINIFLAR)
// =========================================================================

// 1. 📝 DERS NOTU: 4 KADEMELİ DİKEY KATEGORİZE AKORDEON YAPISI
function renderGradeDersNotuAccordion(grade, subData) {
    ...
    const labItemsMap = {
        "5": [
            ...
            { id: "lab-5-2", title: "Güneş, Dünya ve Ay 3D Yörünge Simülatörü", badge: "PhET Simülasyon", desc: "Ay'ın evrelerini ve Dünya çevresindeki dolanma hareketini 3D keşfedin.", format: "SİMÜLASYON", fileUrl: "https://phet.colorado.edu" }
        ],
        "6": [
            { id: "lab-6-1", title: "Bileşke Kuvvet ve Sürtünme Simülatörü", badge: "PhET Simülasyon", desc: "Kuvvet vektörleri, net kuvvet hesabı ve hareket gözlemi.", format: "SİMÜLASYON", fileUrl: "https://phet.colorado.edu" },
            { id: "lab-6-2", title: "Destek ve Hareket / Kemik & Eklem İnceleme Deney Föyü", badge: "Deney Föyü", desc: "Kemik çeşitleri, eklemler ve kasların çalışma prensibi laboratuvar föyü.", format: "PDF FÖY", fileUrl: "#" }
        ],
        "7": [
            { id: "lab-7-1", title: "PhET Enerji Kaykay Parkı & Kinetik-Potansiyel Simülatörü", badge: "PhET Simülasyon", desc: "Kinetik ve potansiyel enerji dönüşümlerini sürtünmeli ortamda keşfedin.", format: "SİMÜLASYON", fileUrl: "https://phet.colorado.edu" },
            { id: "lab-7-2", title: "Mitoz & Mayoz Bölünme Mikroskop Gözlem Föyü", badge: "Deney Föyü", desc: "Bitki ve hayvan hücresi bölünme evreleri laboratuvar inceleme rehberi.", format: "PDF FÖY", fileUrl: "#" }
        ],
        "8": [
            { id: "lab-8-1", title: "Sıvı Basıncı, Derinlik & Yoğunluk Simülatörü", badge: "PhET Simülasyon", desc: "Pascal prensibi, U borusu ve sıvı basıncını etkileyen değişkenler deneyi.", format: "SİMÜLASYON", fileUrl: "https://phet.colorado.edu" },
            { id: "lab-8-2", title: "LGS Bağımlı, Bağımsız & Kontrol Edilen Değişken Kılavuzu", badge: "LGS Deney Modeli", desc: "MEB sınavlarında çıkan deneysel hipotez ve düzenek çözümleri.", format: "PDF / LGS", fileUrl: "#" }
        ]
    };
            ...
            <div class="bg-white/90 backdrop-blur-md border border-slate-200/90 rounded-3xl p-2 sm:p-3 mb-6 shadow-sm">
                <div class="grid grid-cols-3 sm:grid-cols-5 lg:grid-flow-col lg:auto-cols-fr gap-1 sm:gap-1.5 w-full">
                    <button type="button" onclick="filterDersNotuUnits('kitap')" data-unit="kitap" data-active="${isKitapActive ? 'true' : 'false'}" class="notu-filter-btn w-full min-w-0 px-1.5 sm:px-2 py-2 sm:py-2.5 lg:py-3 rounded-xl sm:rounded-2xl text-center transition-all flex flex-col justify-center items-center gap-0.5 ${isKitapActive ? 'shadow-md bg-slate-900 text-white ring-2 ring-slate-900/20' : 'bg-amber-50 text-amber-800 hover:bg-amber-100 border border-amber-200 shadow-sm'} active:scale-95 cursor-pointer">
                        ...
                        <span class="text-[9px] sm:text-[10px] font-semibold opacity-75 truncate w-full">MEB Kitap</span>
                    </button>
...
function renderGradeUnitBasedHub(grade, subData, subTab) {
    ...
    const sectionConfigs = {
        "ders-sunumu": {
            title: `${grade.number}. Sınıf Akıllı Tahta Ders Sunumları`,
            description: "MEB kazanımlarına uygun, sınıf içi projeksiyon ve akıllı tahta uyumlu animasyonlu sunum slaytları.",
            ...
            btnText: "Sunumu Başlat",
            badgeStyle: "bg-orange-50 text-orange-700 border-orange-200",
            defaultFormat: "PPTX / SLAYT",
            btnAction: (item) => `toggleSmartboardMode(true)`
        },
        "videolar": {
            title: `${grade.number}. Sınıf Video Dersler & Deneyler`,
            description: "Fen kavramlarını somutlaştıran konu anlatım videoları, 3D animasyonlar ve laboratuvar çekimleri.",
        ...
        "etkinlikler": {
            title: `${grade.number}. Sınıf Çalışma Föyleri & Etkinlikler`,
            description: "İstasyon etkinlikleri, boşluk doldurma, kavram eşleştirme ve A4 yazdırılabilir çalışma kağıtları.",
        ...
        "soru-bankasi": {
            title: `${grade.number}. Sınıf Soru Bankası & Testler`,
            description: "MEB kazanım değerlendirme testleri, grafik ve tablo yorumlama ağırlıklı beceri temelli sorular.",
        ...
        "denemeler": {
            title: `${grade.number}. Sınıf Ünite Denemeleri & Ortak Sınavlar`,
            description: "Süre tutularak uygulanan dönemlik ortak sınav senaryoları ve Türkiye geneli branş denemeleri.",
        ...
        "egitsel-oyunlar": {
            title: `${grade.number}. Sınıf Eğitsel Oyunlar & Simülasyonlar`,
            description: "Eğlenerek öğrenmeyi sağlayan interaktif Wordwall yarışmaları, eşleştirme oyunları ve fen çarkıfeleği.",
            gradient: "from-fuchsia-600 via-pink-600 to-slate-900",
            icon: "fa-solid fa-gamepad",
            colorText: "text-fuchsia-600",
            btnBg: "bg-fuchsia-600 hover:bg-fuchsia-700 shadow-fuchsia-600/20",
        ...
        "bilim-insanlari": {
            title: `${grade.number}. Sınıf Bilimin Rotasını Çizenler`,
            description: "Müfredat ünitelerindeki keşiflere imza atan bilim insanlarının yaşamları, deneyleri ve modelleri.",
            gradient: "from-cyan-700 via-blue-800 to-slate-950",
            icon: "fa-solid fa-telescope",
            colorText: "text-cyan-700",
            btnBg: "bg-cyan-700 hover:bg-cyan-800 shadow-cyan-700/20",
    ...
    // Laboratuvar öğeleri (Videolar, Etkinlikler, Eğitsel Oyunlar sekmeleri için)
    const labGuideItem = customList.find(m => m && m.id === `lab-${grade.number}-guide`) || {};
    const labSimItem = customList.find(m => m && m.id === `lab-${grade.number}-sim`) || {};
    ...
    return `
        <div id="${containerId}" class="mb-10 animate-in fade-in duration-300">
            ...
            <div class="bg-white/90 backdrop-blur-md border border-slate-200/90 rounded-3xl p-2 sm:p-3 mb-6 shadow-sm">
                <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-flow-col lg:auto-cols-fr gap-1 sm:gap-1.5 w-full">
                    ${unitList.map((uTitle, idx) => {
                        const num = idx + 1;
                        const uName = uTitle.includes(":") ? uTitle.split(":")[1].trim() : uTitle;
            ...
            <!-- Özel materyaller doğrudan kendi ünite akordeonlarına yerleştirildi -->
            ...
            <div class="space-y-4">
                ${unitList.map((uTitle, idx) => {
                    ...
                    const unitCustoms = customList.filter(m => {
                        const gClean = String(m.grade || "").replace(/^grade-/, "").trim().toLowerCase();
                        if (gClean !== "all" && gClean !== String(grade.number)) return false;
                        if (!matchesSubTabCategory(m, normSubTab)) return false;
                        const targetSec = getMaterialTargetSection(m);
                    ...
                    return `
                        <div data-unit="${unitNum}" class="unit-accordion-card ${isOpenInitial ? '' : 'hidden'} border border-slate-200 rounded-3xl bg-white shadow-sm overflow-hidden transition-all duration-200 hover:border-slate-300 hover:shadow-md">
                            ...
                            <button type="button" onclick="toggleAccordionSection('${accordionId}')" class="w-full p-4 sm:p-5 flex items-center justify-between gap-3 text-left transition-colors hover:bg-slate-50">
                                ...
                                <div id="${accordionId}-icon" class="unit-card-icon accordion-icon-rotatable w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 transition-transform duration-300 ${isOpenInitial ? 'rotate-180' : ''}">
                                    <i class="fa-solid fa-chevron-down text-xs"></i>
                                </div>
                            </button>
...
window.filterDersNotuUnits = function(unitIndex) {
    ...
    btns.forEach(b => {
        ...
        if (bUnit === String(unitIndex)) {
            ...
            b.className = "notu-filter-btn w-full min-w-0 px-1.5 sm:px-2 py-2 sm:py-2.5 lg:py-3 rounded-xl sm:rounded-2xl text-center transition-all flex flex-col justify-center items-center gap-0.5 shadow-md bg-slate-900 text-white ring-2 ring-slate-900/20 active:scale-95 cursor-pointer";
            if (descSpan) descSpan.className = "text-[9px] sm:text-[10px] font-medium text-slate-300 truncate w-full";
        } else {
            b.removeAttribute("data-active");
            if (bUnit === "kitap") {
                b.className = "notu-filter-btn w-full min-w-0 px-1.5 sm:px-2 py-2 sm:py-2.5 lg:py-3 rounded-xl sm:rounded-2xl text-center transition-all flex flex-col justify-center items-center gap-0.5 bg-amber-50 text-amber-800 hover:bg-amber-100 border border-amber-200 shadow-sm active:scale-95 cursor-pointer";
                if (descSpan) descSpan.className = "text-[9px] sm:text-[10px] font-semibold text-amber-700/80 truncate w-full";
            } else if (bUnit === "lab") {
                b.className = "notu-filter-btn w-full min-w-0 px-1.5 sm:px-2 py-2 sm:py-2.5 lg:py-3 rounded-xl sm:rounded-2xl text-center transition-all flex flex-col justify-center items-center gap-0.5 bg-emerald-50 text-emerald-800 hover:bg-emerald-100 border border-emerald-200 shadow-sm active:scale-95 cursor-pointer";
                if (descSpan) descSpan.className = "text-[9px] sm:text-[10px] font-semibold text-emerald-700/80 truncate w-full";
            } else {
                b.className = "notu-filter-btn w-full min-w-0 px-1.5 sm:px-2 py-2 sm:py-2.5 lg:py-3 rounded-xl sm:rounded-2xl text-center transition-all flex flex-col justify-center items-center gap-0.5 bg-white text-slate-800 hover:bg-slate-100 hover:border-slate-300 border border-slate-200 shadow-sm active:scale-95 cursor-pointer";
                if (descSpan) descSpan.className = "text-[9px] sm:text-[10px] font-medium text-slate-500 truncate w-full";
            }
        }
    });
